...
<?php
        $liqry = $con->prepare("SELECT title, author, pages, overview, op_voorraad FROM boeken ");
        ...
        $columns = array('title', 'author', 'pages', 'overview', 'op_voorraad');
        ...
        if($liqry === false) {
           echo mysqli_error($con);
            ...
            $liqry->bind_result( $title, $author, $pages, $overview, $op_voorraad);
            if($liqry->execute()){
                $liqry->store_result();
                // while($liqry->fetch()) {
